...
import { Box, Text, Container, Flex, Heading, Select, Stack, Switch, useToast, Link } from "@chakra-ui/react";
...
export default memo(function NotePage() {
  ...
  const newNote = useCallback(async function newNote(title: string) {
    const id = `note_${genHash()}`;
    await dataAgent.newNote(id, title, '', currentFolder);
    navigate(`/app/write/${id}`);
  }, [title, currentFolder]);
  const openNote = useCallback(async function openNote(id: string) {
    const note = await dataAgent.getNote(id);
    setCurrentNote(note);
    const content = note.content || '';
    setText(content);
  }, [setText, setCurrentNote]);
  function toEditNote() {
    if (currentNote?.id) navigate(`/app/write/${currentNote?.id}`);
  }
  const renameNote = useCallback(async function renameNote(id: string, title: string) {
    ...
    setNoteList(notes);
  }, [title, noteList, setNoteList]);
  const moveNote = useCallback(async function moveNote(id: string, folder: string) {
    ...
    setFolderList(folders);
  }, [folder, folderList, setFolderList, noteList, setNoteList]);
  const delNote = useCallback(async function delNote(id: string) {
    console.log("del: ", id);
  ...
  return <Flex direction="column" h="100vh" overflow="hidden" bgColor={darkMode ? "#1e1e1e" : "white"} color={darkMode ? "#cbcaca" : "inherit"}>
      <Flex direction="row" w="100%" overflow="hidden" placeContent="center center" bgColor={darkMode ? "#575759" : "gray.200"}>
      ...
      base: 'column',
      md: 'row'
    }} wrap="wrap" overflow="auto">
        <Container w="xs" h="100%" bgColor={darkMode ? "#252526" : "#f3f3f3"} overflowY="auto" lineHeight={1.4} py={4}>
          <Flex justifyContent="space-between" mt={4} mb={1.5} w="full">
            <Heading size="sm">Dark Mode</Heading>
            <Switch isChecked={darkMode} onChange={handleDarkMode} />
          </Flex>
          <NewNote onNewNote={newNote} darkMode={darkMode} />
          <Select size="sm" mt={2} bgColor={darkMode ? "#3c3c3c" : "white"} borderColor={darkMode ? "#3c3c3c" : "white"} value={currentFolder} onChange={event => getNotesByFolder(event.target.value)}>
            {folderList.map(folder => <option key={folder} value={folder} style={{
            color: "black"
          }}>
                Folder: {folder}
              </option>)}
          ...
          <Stack justifyItems="start" spacing={0} mb={1.5} fontSize="sm">
            {noteList.map(note => <NoteItem key={note.id} note={note} onOpenNote={openNote} onRename={renameNote} onMoveNote={moveNote} onDelNote={delNote} darkMode={darkMode} isActive={note.id === currentNote?.id} />)}
          </Stack>
        </Container>
        <Box flex={1} minH="100%" h="100%" overflow="auto" key={currentNote?.id || 'blank'}>
          <Heading size="xl" m={2}>{currentNote?.title || ''}</Heading>
          <Box overflow="auto">
            <Preview text={text} darkMode={darkMode} wikiBase={"app/notes"} tagBase={"app/tag"} />
          </Box>
        </Box>
      </Flex>
      <Footer />
