...
export const ArticleView = memo(function ArticleView(props: ViewProps) {
  const {
    article,
    starArticle
  } = props;
  ...
  useEffect(() => {
    if (article) {
      try {
        extract(article.feed_url, undefined, {
          proxy: {
            target: '/proxy/gethtml?url='
          }
        }).then(articleData => {
          const content = (articleData?.content || article.intro || "").replace(/<a[^>]+>/gi, (a: string) => {
  ...
  return <Box h="full">
      <div className="px-2 mb-1">
        ...
        <HStack className="flex items-center justify-start">
          ...
          <a className="m-1 dark:text-slate-400" target="_blank" rel="noreferrer" href={feed_url}>
            <TbLink size={20} />
          </a>
          <Button size="xs" title={`${isStar ? 'Starred' : 'Star'}`} onClick={async () => {
          await starArticle(article.feed_url, Math.abs(Number(isStar) - 1));
          setIsStar(!isStar);
        }}>
            <TbStar size={20} color={`${isStar ? 'red' : 'green'}`} />
          </Button>
          {article.audio_url.trim() && <Button size="xs" onClick={() => setCurrentPod(articleToPod(article))}>
