...
export default memo(function Feed() {
  ...
  const getList = () => {
    Promise.all([dataAgent.getChannels(), dataAgent.getUnreadNum()]).then(([channels, unreadNum]) => {
  ...
  const refreshList = useCallback(async () => {
    ...
    for (const channel of channelList) {
      ...
      setDoneNum(doneNumCurrent => doneNumCurrent + 1);
    }
    setRefreshing(false);
  }, [channelList, setRefreshing, setDoneNum]);
  const onShowManager = useCallback(() => {
    setShowManager(showManagerCurrent => !showManagerCurrent);
  }, [setShowManager]);
  ...
  const onClickFeed = useCallback(async (link: string) => {
    setLoading(true);
    ...
    setLoading(false);
  }, [channelList, setLoading, setCurrentChannel, setShowManager]);
  const onBeforeClick = () => {
    setLoading(true);
  ...
  const onClickStar = useCallback(async () => {
    ...
    setLoading(false);
  }, [setLoading, setCurrentArticles, setStarChannel]);
  const onClickAudio = useCallback(async () => {
    onBeforeClick();
    const audioArticles = await dataAgent.getAudioArticles();
    setCurrentArticles(audioArticles);
    setLoading(false);
  }, [setLoading, setCurrentArticles]);
  const handleAddFeed = useCallback(async (feedUrl: string, ty: string, title: string) => {
    const res = await dataAgent.addChannel(feedUrl, ty, title);
    if (res > 0) {
      getList();
    }
  }, [title, feedUrl]);
  const handleDeleteFeed = useCallback(async (channel: ChannelType) => {
  ...
  const handleRefresh = useCallback(async () => {
    ...
    setSyncing(false);
  }, [currentChannel, setSyncing]);
  const updateAllReadStatus = useCallback(async (feedLink: string, status: number) => {
    const res = await dataAgent.updateAllReadStatus(feedLink, status);
    if (res === 0) return;
    getList();
    await handleRefresh();
  }, []);
  const onClickArticle = useCallback(async (article: ArticleType) => {
  ...
  const updateStarStatus = useCallback(async (url: string, status: number) => {
      ...
      <Flex direction="row" h="100vh" overflow="auto" m={2}>
        <Split className="split" sizes={[20, 80]} minSize={50}>
          <Box minW={64} h="full" p={1} overflowY="auto">
            ...
            <ChannelList channelList={channelList} refreshList={refreshList} onShowManager={onShowManager} onClickFeed={onClickFeed} onClickStar={onClickStar} onClickAudio={onClickAudio} refreshing={refreshing} doneNum={doneNum} />
          </Box>
          {showManager ? <Box w="full" overflowY="auto" m={1} p={2}>
              <FeedManager channelList={channelList} handleAddFeed={handleAddFeed} handleDelete={handleDeleteFeed} />
            </Box> : <Split className="split" sizes={[25, 75]} minSize={50}>
              <Box overflowY="auto" minW={72}>
                <Channel channel={currentChannel} starChannel={starChannel} articles={currentArticles} handleRefresh={handleRefresh} updateAllReadStatus={updateAllReadStatus} onClickArticle={onClickArticle} loading={loading} syncing={syncing} />
              </Box>
              <Box flex={1} overflowY="auto">
                <ArticleView article={currentArticle || storeArticle} starArticle={updateStarStatus} />
              </Box>
            </Split>}
        </Split>
      </Flex>
